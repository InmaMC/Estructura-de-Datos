/**
  ...
  */
#include <cassert>
#include <Queue.h>  //El codigo ya se incluye en queue.h
...
void Cola<T>::ponerFrente(const T & elem){
  ...
  if (primera==0)                    //Si la lista está vacía,
    ...
    aux->siguiente = primera; //Actualizamos el puntero siguiente del aux al frente
    primera = aux;            //Actualizamos primera para que apunte a aux
  }
  num_elem++;                //Incrementamos el numero de elementos
}
